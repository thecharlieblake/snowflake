...
def construct_graph_input_feeddict(
    node_info,
    obs_n,
    receive_idx,
    send_idx,
    ...
    assert len(request_data) > 0 and np.all(
        [dtype in ["ob", "idx"] for dtype in request_data]
    ...
    if "ob" in request_data:
        ...
        current_batch_size = obs_n.shape[0]
        graph_ob, graph_parameters = _get_obs(node_info, obs_n, current_batch_size)
...
def _construct_index(node_info, batch_size):
    ...
    """
    logger.info(
        "Building new send_idx and receive_idx for the model, "
        + "current batch size: {}".format(batch_size)
    )
    # size [batch_size]
    offset = node_info["num_nodes"] * np.array(range(batch_size))
    ...
    send_idx = {}
    for i_edge_type in node_info["edge_type_list"]:
        send_idx[i_edge_type] = _add_offset(
            node_info["send_idx"][i_edge_type], offset, batch_size, if_reshape=True
        )
...
def _get_obs(node_info, raw_ob, batch_size):
    ...
    for node_type in node_info["node_type_dict"]:
        ...
        for node_pos_in_graph_ob, i_node_id in enumerate(
            ...
            graph_ob[node_type][
                :, node_pos_in_graph_ob, : len(node_info["input_dict"][i_node_id])
            ] = raw_ob[:, node_info["input_dict"][i_node_id]]
    ...
    for node_type in node_info["node_type_dict"]:
        ...
        if node_info["para_size_dict"][node_type] == 0:
            ...
            assert this_shape[2] == 0, logger.error("wrong ob shape!")
            graph_parameters[node_type] = np.zeros([this_shape[0] * this_shape[1], 0])
        else:
...
def _add_offset(idx, offset, batch_size, if_reshape):
    ...
    tiled_idx = tiled_idx + (np.tile(offset, [tiled_idx.shape[1], 1])).transpose()
...
def _add_inverse_offset(extype_offset, intype_offset, self_offset, batch_size):
    ...
    incremental_offset *= np.expand_dims(list(range(batch_size)), 1)
