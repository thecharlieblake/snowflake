...
def map_output(transfer_env, i_value, added_constant, gnn_option_list):
    ...
    for output_id, output_node_id in enumerate(oenv_info["output_list"]):
        ...
        if node_name in ienv_node_name_list:
            if ienv_node_name_list.index(node_name) not in ienv_info["output_list"]:
                logger.warning("Missing joint: {}".format(node_name))
                continue
            o_value[output_id] = i_value[
                ienv_info["output_list"].index(ienv_node_name_list.index(node_name))
            ]
        else:
            ...
            for i_node_id in repetitive_struct_node_list:
                o_value[output_id] += (
                    i_value[ienv_info["output_list"].index(i_node_id)]
                    / num_reptitive_nodes
                )
    ...
    for output_id, output_node_id in oenv_info["input_dict"].items():
        ...
        if node_name in ienv_node_name_list:
            o_value[output_node_id] = i_value[
                ienv_info["input_dict"][ienv_node_name_list.index(node_name)]
            ]
        else:
            continue
    return o_value
...
def map_transfer_env_running_mean(
    ienv,
    oenv,
    running_mean_info,
    observation_size,
    ...
    for node, oenv_digit in oenv_info["input_dict"].items():
        ...
        # if the node is alreay in the input environment?
        if node_name in ienv_node_name_list:
            ienv_digit = ienv_info["input_dict"][ienv_node_name_list.index(node_name)]
            ...
            for key in ["mean", "variance"]:
                o_running_mean_info[key][oenv_digit] = i_running_mean_info[key][
                    ienv_digit
                ]
        else:
            # the name format: "@type_@name_@number", e.g.: joint_leg_1
            ...
            for i_node_id in repetitive_struct_node_list:
                ...
                for key in ["mean", "variance"]:
                    o_running_mean_info[key][oenv_digit] += (
                        i_running_mean_info[key][ienv_digit] / num_reptitive_nodes
                    )
